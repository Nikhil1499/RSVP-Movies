...
-- Ans- The highest number of movies have been produced in the year of 2017. More movies have been produced in the past year than in the recent years.
...
-- "Drama" genre has the highest number of movies overall
...
-- "Action" genre has the highest average duration (112.88) of movies
...
inner join 
	...
	on m.id = g.movie_id
where 
	m.country='USA' and r.total_votes>1000 and month(date_published)=3 and year=2017
group by 
	g.genre
order by 
	movie_count desc;
...
select 
	...
	on m.id = r.movie_id
	where m.languages like "%German%") as German_votes,
	(select 
		sum(total_votes)
	from 
		movie as m
	inner join ratings as r
	on m.id = r.movie_id
	where m.languages like "%Italian%")as Italian_votes;
...
with top_genres as
(
select g.genre,count( distinct g.movie_id) as mv_cnt
from genre as g
inner join
ratings as r
on g.movie_id=r.movie_id
